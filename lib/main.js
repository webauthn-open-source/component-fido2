...
module.exports = class Fido2Component extends Component {
    constructor(cm) {
        ...
        this.defaultCreateOptions = CreateOptions.from({
            timeout: this.timeout,
            rp: {},
            user: {},
            // TODO: crypto algorithms supported should be provided by fido2lib
            // preferences / order should be offered by config
            pubKeyCredParams: [{
                type: "public-key",
                alg: -7
            }]
    ...
    async registerRequest(req, res) {
        try {
            ...
            const {
                challenge
            } = await this.fido2lib.createCredentialChallenge();
            ...
            // serverIcon
            // userIcon
            var respOpts = CreateOptions.from(this.defaultCreateOptions.toObject());
            respOpts.challenge = challenge;
            respOpts.rp.name = this.serviceName;
            respOpts.user.name = optReq.username;
            respOpts.user.id = optReq.username.split("").map((ch) => ch.charCodeAt(0));
            respOpts.user.displayName = optReq.displayName;
            respOpts.status = "ok";

            // save data to session
            req.session.username = optReq.username;
            req.session.registerChallenge = coerceToBase64(challenge);
    ...
    async registerResponse(req, res) {
        try {
            ...
            req.session.registerChallenge = coerceToArrayBuffer(req.session.registerChallenge);
            if (!req.session ||
                typeof req.session.username !== "string" ||
                typeof req.session.registerChallengeTime !== "number" ||
                !(req.session.registerChallenge instanceof ArrayBuffer)) {

                throw new Error("malformed session information");
            }
            ...
            var result = await this.fido2lib.createCredentialResponse(attResult.toObject(), challenge, this.origin, "either");
    ...
    async loginRequest(req, res, next) {
        try {
            ...
            var {
                challenge
            } = await this.fido2lib.getAssertionChallenge();
    ...
    async loginResponse(req, res, next) {
        try {
            ...
            req.session.loginChallenge = coerceToArrayBuffer(req.session.loginChallenge);
            if (!req.session ||
                typeof req.session.username !== "string" ||
                typeof req.session.loginChallengeTime !== "number" ||
                !(req.session.loginChallenge instanceof ArrayBuffer)) {
                throw new Error("malformed session information");
            }
            var username = req.session.username;
            ...
            if (creds.length !== 1) {
                throw new Error("error finding credential ID: " + assn.rawId);
            }
